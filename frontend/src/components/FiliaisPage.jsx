...
function LinhaFilial({ filial, podeExcluir, onSalvo, onExcluido }) {
  ...
  async function salvar() {
    ...
    setErro("");
    try {
      await api.atualizarFilial(filial.id, { nome, endereco });
      setEditando(false);
      onSalvo();
    } catch (e) {
  ...
  async function excluir() {
    if (!confirm(`Excluir a filial "${filial.nome}"?`)) return;
    try {
      await api.excluirFilial(filial.id);
      onExcluido();
    } catch (e) {
  ...
  if (!editando) {
    return (
      <tr className="border-b border-panelborder/50 last:border-0">
        <td className="py-2 pr-2 text-gray-200">{filial.nome}</td>
        <td className="py-2 pr-2 text-gray-400">{filial.endereco || "—"}</td>
        <td className="py-2">
  ...
  return (
    <tr className="border-b border-panelborder/50 last:border-0 bg-panelalt/30 align-top">
      ...
      <td className="py-2 pr-2">
        <input className={`${campoCls} w-full`} value={endereco} onChange={(e) => setEndereco(e.target.value)} />
      </td>
      <td className="py-2">
...
function NovaFilial({ onCriada }) {
  ...
  async function criar() {
    ...
    setErro("");
    try {
      await api.criarFilial({ nome, endereco });
      setNome("");
      setEndereco("");
      setAberto(false);
      onCriada();
  ...
  return (
    <div className="bg-panelalt border border-panelborder rounded-lg p-3 flex flex-wrap items-end gap-2">
      <div>
        ...
        <input className={campoCls} value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Ex.: Matriz" />
      </div>
      <div>
        <label className="block text-[11px] text-gray-500 mb-1">Endereço</label>
        <input className={campoCls} value={endereco} onChange={(e) => setEndereco(e.target.value)} placeholder="Opcional" />
      </div>
      <button
        onClick={criar}
        disabled={salvando}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-white font-semibold">Filiais</h2>
      ...
      <div className="bg-panel border border-panelborder rounded-xl p-4 overflow-x-auto">
        {carregando ? (
          ...
          <p className="text-gray-500 text-sm">Nenhuma filial cadastrada ainda.</p>
        ) : (
          <table className="w-full text-sm min-w-[480px]">
            ...
            <tbody>
              {filiais.map((f) => (
                <LinhaFilial key={f.id} filial={f} podeExcluir={podeExcluir} onSalvo={carregar} onExcluido={carregar} />
              ))}
            </tbody>
          </table>
        )}
